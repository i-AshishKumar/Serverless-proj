...
def publish_concern(request):
    ...
    if not request_json or not all(field in request_json for field in required_fields):
        return (jsonify({'success': False, 'error': 'User ID, Booking Reference, and Message are required'}), 400, headers)

    user_id = request_json['user_id']
    booking_reference = request_json['booking_reference']
    message = request_json['message']
    assigned_agent = random.choice(['agent1', 'agent2', 'agent3'])
    ...
    return (jsonify({'success': True, 'assigned_agent': assigned_agent}), 200, headers)